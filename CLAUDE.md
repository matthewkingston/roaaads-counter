...
python3 analysis/derive_component_profiles.py              # regenerate hourly_fractions.csv component columns (re-run when NTS files change)
python3 analysis/derive_generation_rates.py                # regenerate generation_rates.json — NTS0409a vehicle-driver trips/person/day (re-run when NTS0409 or the purpose mapping changes)
...
**Legacy unconstrained path:** the old unconstrained product `T_ij = K × w_i × w_j × f(d_ij)` with a
combined node weight `w = population + W_BIZ × business_demand` survives only in `model.gravity_assign`
for back-compatibility with pre-split param files; the production-constrained components supersede it.
...
with **no weight parameter and no self/cross term** (the old single business component's `W_BIZ`
and biz×biz term are gone):
...
The self-term applies to every component. The **school** self-term is now **active** (external
`school_demand` and `school_producers` are populated per zone), so the school denominators carry the
intra-zonal diagonal and external zones retain school trips locally rather than dumping into the core
(the old "whole world goes to school in Newtownards" failure is resolved); commute and retail feed
their denominators from the populated `workplace`/`retail_spaces` layers likewise.

Each component has its own temporal profile and scale (K_res, K_commute, K_retail, K_sch).
...
Using the driver row makes it vehicles by construction and obviates the old all-mode `EDU_FACTOR`.
...
calibrated **directly** by `solve_scales` (no `(K, φ)` reparam, no `f`-steps, no alternation). The
temporal fractions `f_res/f_commute/f_retail/f_school` are **pinned at the NTS profile**
(`hourly_fractions.csv` `mean_fraction_*`) and never tuned — so every observation prediction is
**linear in the scales**: `pred_i = Σ_c K_c·a^c_i` with `a^res_i = m_res_i·Th_i·f_res[s_i]` constant
(commute/retail/school analogously). The inner objective — Gaussian WLS over the 216 official obs + Poisson **identity-link**
...
global df — gravity shape params + 4 scales — are not subtracted, per the prior convention that only
...
- `build_assignment.py` "Official count sites" block and the `"flows"` values in
  ...
  where `W_c` (`model.aadt_weights`, a 4-tuple summing to ~1) is the day-type-weighted
  (5·weekday+Sat+Sun)/7 sum of component `c`'s hourly fractions. **The `W_c` weighting is essential:**
  `K_c·flow_c` is calibrated so `K_c·flow_c·f_c[slot]` matches the *hourly* count, so `K_c·flow_c`
  alone is NOT a daily total — it is ~1/ΣW ≈ 2.6× too large. (The unweighted per-component flows still
  feed `compute_chi2`, which applies `f_c` itself — do not double-weight.) `newtownards_flows.json`
  stores the W-weighted AADT in `flows`/`flows_res`/`flows_commute`/`flows_retail`/`flows_school` plus
  an `aadt_weights` block. Node-based sites (508/444) sum every directed link at the node.
...
- Walking "Model" column convention (display only, chi²/N unaffected): both `compute_chi2()`
  ...
  factor, so it showed AADT×Th; the tuner used a K-weighted reconstruction. Both now use the sum of
...
**Four-component split + generation pinning (current model state).** The business component was
split into independent, separately-scaled **commute** (producer `commute_producers` → attractor
`workplace`) and **retail** (producer population → attractor `retail_spaces`) production-constrained
components, each a clone of school; `W_BIZ` and the biz×biz term were removed. The model now tunes
**9 shape params** and four scales `K_res/K_commute/K_retail/K_sch` (see "Four-component flow
decomposition"). Generation is then **pinned** to NTS0409a vehicle-driver trips/day via
`model.compute_generation_scales` (island per-capita anchors), so each `K_c` is a **≈1 verification
anchor** rather than a free magnitude (see "Generation pinning"). **No 4-component / generation-pinned
tune has been run yet** — the paths cache is stale on the OSRM-profile inputs, so the headline
`K_c ≈ 1` check awaits the `reduce_deadends → build_paths → tune` rebuild.
...
- **Paths cache stale** — rebuild with `build_paths.py` before re-tuning (currently stale on the tuned-profile / base-speeds inputs, per the staleness guard).
...
- **Scale-share degeneracy:** the convex `solve_scales` could otherwise trade magnitude between
  ...
  shares (`φ = K_c/ΣK`). `W_BIZ` and the biz×biz term are gone, so there is no longer a
  `K_biz × W_BIZ` collapse mode. With generation pinning the shares are also largely set a priori by
  the NTS rates `ρ_c`.
- **Component scales** `K_res`/`K_commute`/`K_retail`/`K_sch` are calibrated directly by the convex
  `solve_scales` at each optimizer step (no combined `K`, no `(K,φ)` reparam). With generation pinned
  to vehicle-driver trips/day each `K_c ≈ 1` (a verification anchor); a deviation diagnoses local
  car-mobilisation vs the national average. chi²/N is the reliable fit metric.
...
- **Temporal profiles are pinned, not inferred:** `f_res/f_commute/f_retail/f_school` are fixed at
  ...
  consumed (N_eff = N = 890). The old per-slot f-inference and `gamma_coupling_scale` aggregate
  coupling are retired.
...
- **`tuned_params.json` structure:** contains the four scales `K_res`/`K_commute`/`K_retail`/`K_sch`, the 9 shape params (P, ALPHA, BETA, P_commute, ALPHA_commute, P_retail, ALPHA_retail, P_school, ALPHA_school), and `slot_fracs_res`/`slot_fracs_commute`/`slot_fracs_retail`/`slot_fracs_school` (dicts keyed `"dt,h"`, the pinned NTS profile). No `W_BIZ`/`W_SCHOOL`/`K_biz`/`P_biz`/`ALPHA_biz`, no legacy combined `K`, no `external_node_*`/`dampings` keys. `reset_gravity_params.py` regenerates this clean structure; old pre-split param files fall back to the legacy `gravity_assign` mode in `build_assignment.py`.
