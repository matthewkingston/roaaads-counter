...
| `analysis/hourly_fractions.csv` | **Tracked in git.** NI-average hourly fraction profile, one column per flow component: `mean_fraction_res`, `mean_fraction_commute`, `mean_fraction_retail`, `mean_fraction_school`. **Derived from NTS** via `analysis/derive_component_profiles.py`. Constraint: the component columns sum to the aggregate `mean_fraction` for all 168 rows; rows sum to 7.0 (AADT normalisation). Re-run `derive_component_profiles.py` whenever NTS files change. |
| `analysis/derive_component_profiles.py` | Derives the per-component hourly fraction columns (`mean_fraction_res`/`_commute`/`_retail`/`_school`) from DfT NTS data (2023–2024 rolling average) via a purpose→component classification. Re-run whenever NTS files or the purpose classification change. |
