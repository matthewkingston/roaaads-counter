...
- **School** (`flow_school`): symmetric pop↔school split, per-origin normalised, (P_school,
  ...
  Magnitude is `K_sch` (no `W_SCHOOL`). **KNOWN-BROKEN:** external nodes have `school_demand=0`, so
  every external origin's school budget dumps into the core ("the whole world goes to school in
  Newtownards") — a missing-external-school-demand data gap, not a code bug (see memory note).
