...
| `analysis/derive_generation_rates.py` | **Tracked output `analysis/generation_rates.json`.** Derives per-component vehicle-driver trips/person/day from England **NTS0409a** (`data/nts0409.ods`, 2023/24 avg, modes *Car or van driver + Motorcycle + Taxi or minicab*) → the generation-pinning rates `ρ_c` (commute 0.178, retail 0.495, res 0.262). The school total `ρ_school` (0.082) is **split into three explicit per-level rates** — `school_primary` (0.0349), `school_postprimary` (0.0292), `school_tertiary` (0.0178) — by each level's island enrolment share (`census_school_producers.island_enrolment_by_level()`), so each school level is a fully independent generation component (a change to one level cannot perturb the others). Encodes the **purpose→component map** and `LEISURE_RETAIL_FRAC = 0.5` as named, commented module constants (the judgment allocations — see "Generation pinning"). Re-run when the NTS file, the mapping, or the enrolment split changes. Consumed via `model.load_generation_rates` / `model.compute_generation_scales`. |
...
or minicab*): commute 0.178, retail 0.495, res 0.262 trips/person/day, and the school total 0.082
**split into three explicit per-level rates** by island enrolment share — school_primary 0.0349,
school_postprimary 0.0292, school_tertiary 0.0178 (sum ≈ 1.017). Each level's ρ is a frozen constant
so the levels stay fully decoupled. Using the driver row makes it vehicles by construction.
