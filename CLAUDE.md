...
| `simulation/demographics_config.py` | **NEW.** Shared pure-constant config imported by both `build_demographics.py` and `build_map.py` (study-area centre, file paths, OSM tag handling — `EXCLUDE_AMENITY`/`POI_WEIGHTS`/`SCHOOL_ENROLL_FALLBACK` — and map styling). Single source of truth so the split scripts don't drift. |
