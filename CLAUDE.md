...
- **Mode substitution** (the 0→peak rise) — `driveShare(equiv_miles(c))`: short trips are *walked,
  ...
  **per-component** — the microdata shows the rise genuinely differs by purpose (commute rises
  ...
  driveShare→0`). *(All six curves — incl. the three per-level school curves — are in `CURVES`;
  model._modesub_kernel still passes the shared blend `component=None` and is wired per-component
  next, deleting the `None` fallback, then re-tuned.)*
