...
| `simulation/model.py` | **Shared constants and functions:** `COUNT_SITES`, `EXCLUDE_LINKS`, file-path constants, `gravity_assign()` (rational kernel; `return_components=True` with `w_school` provided returns `(flow_res, flow_biz, flow_school)` tuple), `site_flow()`, `compute_chi2()`, `print_chi2_table()`. `gravity_assign()` accepts optional `link_weight` array (probit fractional weights). `compute_chi2()` dispatches to three-component mode when `link_flow_school_dict` provided (N_eff = N − 3·N_slots); two-component when only `link_flow_biz_dict` provided; legacy single-flow otherwise. **COUNT_SITES `node` values are currently `None` (pending)** — must be updated with OSM node IDs after running `build_network.py` and identifying each count-site node in the map. `EXCLUDE_LINKS` is currently empty (pending re-identification after re-ingestion with OSM IDs). |
...
**Walking counts:** 7 CSV files, 177 sessions. `counts_processed.json` has been wiped and needs re-ingestion with the new OSM-ID graph. `manual_link_overrides.json` has been cleared — all 7 manual overrides (sessions e644eae2, 760b0c8e, e66989f4, bb934ba7, b0043fd1, 32d425d6, 29d5f5f0) must be re-entered using OSM node IDs via `analysis/manual_assign_link.py`.
New sessions added 2026-06-18 (7 sessions): Saratoga Avenue, Glenford Road, Hardford Link, Belfast Road.
**Note: pre-migration observation count was 559 (216 official hourly + 343 walking). Old node references (e.g. link 8→7 for Kempe Stones) are sequential IDs and no longer valid.**
...
- **EXCLUDE_LINKS pending re-identification** — was `(161, 160)` (old sequential IDs, too-short count distorted by traffic lights); must be re-identified after re-ingesting with OSM node IDs.
- **COUNT_SITES pending** — official hourly site nodes must be set to OSM IDs after running `build_network.py` and `build_demographics.py --map-only` then identifying nodes visually.
