...
| `simulation/reset_gravity_params.py` | Reset the gravity params in `tuned_params.json` to the `gravity_ref` anchors in `tuner_config.json`: every `gravity_ref` shape param (TAU, TAU_commute, TAU_retail, THETA, TAU_school — iterates `gravity_ref`, so no rename drift) plus the four scales `K_res/K_commute/K_retail/K_sch` → 1.0. Strips dead keys (`K`, `K_biz`, `W_BIZ`, `W_SCHOOL`, `P_biz`, `ALPHA_biz`, the rational-kernel tail exponents `ALPHA`/`ALPHA_commute`/`ALPHA_retail`/`ALPHA_school`, the Tanner peak/shape `P`/`BETA`/`P_commute`/`BETA_commute`/`P_retail`/`BETA_retail`/`P_school`/`BETA_school`, `MU`, `SIGMA`). External params and `slot_fracs_*` are preserved. |
