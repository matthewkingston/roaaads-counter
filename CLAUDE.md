...
`model._rational_kernel` evaluates this in the algebraically-identical, **overflow-safe** form
`f = (ALPHA+BETA) / (ALPHA·u^(-BETA) + BETA·u^ALPHA)`: the denominator is a sum of two non-negative
powers of u (>0 for all u>0, minimised = ALPHA+BETA at u=1 ⇒ f=1), and at most one term can overflow,
so a power overflow yields f→0 (the correct tail/origin limit) instead of the ∞/∞ = NaN the direct
form produces at large ALPHA+BETA (the optimizer pushing ALPHA high otherwise produced `nan` evals).
