...
- **External node probit loading:** all edges (road and external) receive the same log-normal noise each pass. Route diversity for external-internal OD pairs comes from both the X→B external leg and the internal B→J portion, giving stochastic spread across similarly-weighted boundary entry points.
