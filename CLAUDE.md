...
`b`). This makes a doubly-constrained tuner eval ~a few × singly (measured ~17× faster than
converging, warm k=10 ≈5 s vs cold ≈84 s over all 10 legs) ⇒ **a full re-tune is an overnight run**.
`tune_assignment.py` uses the approximate path (cache persists across the Powell evals);
`build_assignment.py` leaves `furness_max_sweeps=None` so the **deployed** flows converge exactly.
A faster exact solver (Anderson / Newton-CG on the convex balancing dual) could shrink `k` further
but is not needed at k≈10 — a possible follow-up.
