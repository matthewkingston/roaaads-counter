...
python3 analysis/derive_mobilisation.py                    # island car-driver mobilisation level + spatial-dispersion width → analysis/mobilisation.json (m_island rescales gen rates to island level; sigma_mob = K-prior common-mode width). NI=TSNI 2023 (455/yr), RoI=CSO NTA04/NTA11 2019 × census pop. Needs the TSNI ODS + data/ireland_nts/ + RoI SAPS + NI NISRA census.
...
python3 analysis/derive_generation_rates.py [--boot N]     # regenerate generation_rates.json — per-capita vehicle-driver trips/person/day from NTS microdata (23-cat B01 mapping); school = per-student × island students/pop; retail += pre-school escort fudge. ALSO writes _meta.stat_cov: a PSU-cluster bootstrap (--boot, default 500) 6×6 log-rate sampling covariance for the K-prior. Re-run when the microdata, mapping, or school rates change.
...
| `simulation/tuner_config.json` | **Tracked in git.** Gravity param regularization and the scale K-prior. `gravity_lambda` + `gravity_ref` regularise the 18 double-exp willingness params (`<comp>_taus`/`_taul`/`_w` for res/commute/retail + the three independent school levels; τ's in seconds — `gravity_ref` holds each component's current `(w, τs, τl)`, no shared school kernel; seeded from the kernel fit by `sync_kernel_anchor.py`); `gravity_fixed` (an optional list of willingness keys held at their `gravity_ref` value and **excluded from the tuned vector** — for pinning individual params; unknown keys warn and are ignored, default `[]`); `K_anchor_floor` sets the two/three floor knobs of the **properly-derived K-prior** `(K−1)ᵀΛ_K(K−1)` inside `solve_scales` (anchor 1, fixed in code). Λ_K = pinv(P⊥·Cov_boot·P⊥ + σ_mob²·uuᵀ + split_sigma²·P⊥): its SHAPE is *derived* — `Cov_boot` is the cluster-bootstrap sampling covariance from `generation_rates.json` `_meta.stat_cov` (differential/split directions, common mode projected out), `σ_mob` is the island mobilisation spatial dispersion from `mobilisation.json` (common-mode/level width — the loose leash the counts refine). `K_anchor_floor.split_sigma` (default 0.1) is the England→Ireland transfer floor on the purpose split (the bootstrap CV is ~1.6%, too confident to transfer the MIX at, so this floors it); `mob_sigma` (default 0, σ_mob usually owns the common mode) and `mob_default` (0.5, used only if `mobilisation.json` is absent) complete it. Replaces the old flat `K_prior_std` (per-component σ=0.5, isotropic, no cross-terms). The `doubly_constrained` list (which components are attraction-constrained via Furness; default `[]` ⇒ singly everywhere — see "Doubly-constrained option") and `furness_max_sweeps` (the approximate-balancing warm-sweep budget `k`, default 12) are carried into `tuned_params.json` by `reset_gravity_params.py`. `gamma_coupling_scale` and `lambda` are unused dead keys left in the file. External demand layers (workplace, retail_spaces, per-level school_demand, commute producers, per-level school producers) are measured per zone in `census_zones.json` — no external scale factors. |
...
| `analysis/ingest_counts.py` | Reads all CSVs from `data/counts/`, snaps GPS tracks to road links, estimates per-session AADT via hourly fraction profile. Idempotent: skips already-processed sessions. Loads manual link overrides from `data/manual_link_overrides.json`. After every new link assignment, validates each non-null count direction against the directed graph; raises `ValueError` if the edge doesn't exist. **Single-mode zero-count on a one-way road:** a `single`-mode session with **zero cars observed** that snaps to a **one-way** link (exactly one directed edge present in `G`) is attributed a genuine `0` on the one direction the link permits — the direction cars flow, detected via `G.has_edge`, **independent of the observer's walking direction** (the zero lands on the same physical edge either way; the walk direction only sets whether it's stored under the `with`/`against` label). This is the only path that records a real zero (single mode otherwise nulls a no-car session because it can't infer the watched direction). Strictly scoped: a **two-way** link with a zero single-mode session is still dropped (direction unknowable), and a **dual**-mode session on a one-way link still fails loud (non-null `0` on the missing reverse edge → `ValueError`). |
...
| `analysis/derive_generation_rates.py` | **Tracked output `analysis/generation_rates.json`.** Derives per-component **per-capita** vehicle-driver trips/person/day from the **NTS microdata** (via `nts_microdata`, 2023/24, MainMode_B04ID {3,5,12}, Σ(JJXSC×W5)÷Σ W2 persons ÷7) using the 23-cat `purpose_mapping.B01_COMPONENT` mapping → commute 0.200, retail 0.581, res 0.159. **School is per-capita converted from the per-student rates** (`school_generation_rates.json`) × (island `node_school_producers_<level>`/`node_population`) so ρ_school/k_students recovers the per-student rate exactly → `school_primary` 0.0381, `school_postprimary` 0.0276, `school_tertiary` 0.0025 (tertiary far below the old enrolment-share placeholder — that overstated tertiary ~7×). Retail additionally absorbs the pre-school escort fudge. **Every rate is per-capita** (encodes an island total; the producer/attractor layer only distributes spatially). Reads `node_weights_reduced.json` for the school students/pop ratio, so it is all-Ireland-specific. Re-run when the microdata, the mapping, or the school rates change. Consumed via `model.load_generation_rates` (which **rescales these England rates to the island level by ×`m_island`** from `mobilisation.json`, preserving the split) / `model.compute_generation_scales`. Also writes `_meta.stat_cov` — a **PSU-cluster bootstrap** (`--boot`, default 500; resample PSUs within SurveyYear, threading a household multiplicity through BOTH the B01 rates and the school escort+self-drive derivation) → the 6×6 **log-rate sampling covariance** feeding the K-prior's derived split directions. Sampling CV is tight for res/commute/retail (~1.6%) and loose for tertiary (~large); the school students/pop ratio is held fixed. |
| `analysis/derive_mobilisation.py` | **Tracked output `analysis/mobilisation.json`.** Derives the single island car-driver mobilisation **level** (`island_level`), the global rescale factor **`m_island`** = `island_level / Σρ_NTS` (applied by `model.load_generation_rates` so K anchors at 1 against island-level generation), and the K-prior common-mode width **`sigma_mob`** (spatial dispersion of mobilisation). **One island number, NOT a NI/RoI split** (the border is not a behavioural boundary — the difference is a density/rurality confound; a single constant is CENTRE-portable). NI level = TSNI `Table_3_2` 2023 (Car Driver 434 + Motorcycle + Other private + Taxi = 455/person/yr, all ages) ÷365. RoI level = Σ_(age×sex) NTA04 2019 journeys/person/day × RoI SAPS census pop × 0.688 car-driver-ish fraction (NTA11 2019) / RoI total pop (18+ rates over all-ages denominator; under-18 driver gap negligible). Island = population-weighted pool. Mixed vintage (NI 2023 / RoI 2019) is a deliberate best-data-per-jurisdiction call. `sigma_mob` = quadrature of the between-region level CV and a within-region census car-ownership dispersion (RoI SAPS T15_1 household cars; NI person-weighted car availability — units differ, approximate; components stored in the JSON so the recipe can be revised without a re-run). |
...
`analysis/generation_rates.json` — committed; per-component vehicle-driver trips/person/day (**England** level, output of `derive_generation_rates.py`; `model.load_generation_rates` rescales to island by ×`m_island`). Source data for generation pinning (`model.compute_generation_scales`). `_meta.stat_cov` carries the PSU-cluster bootstrap 6×6 log-rate sampling covariance for the K-prior.
`analysis/mobilisation.json` — committed; output of `derive_mobilisation.py`. `island_level` + `m_island` (the single global generation rescale to island car-driver level) + `sigma_mob` (K-prior common-mode/level width) + per-jurisdiction levels and `sigma_mob_components`. Re-run if the TSNI/CSO travel-survey inputs or census populations change.
...
diagnoses local car-mobilisation vs the **island** average, honestly, since the level is now
island-anchored). This is a **model-layer** change (no paths-cache rebuild). Independent of the kernel
form (the mode-substitution × willingness kernel; see memory `project-tanner-kernel-tld`).

**Island mobilisation rescale (level).** The NTS rates below are **England** behaviour; islanders
drive more. `model.load_generation_rates` multiplies every `ρ_c` by a single global **`m_island`**
(`mobilisation.json`, from `derive_mobilisation.py` — ≈1.53: island level ≈1.54 vs England Σρ≈1.01),
preserving the NTS purpose split, so the deployed rates are `island_level × NTS_share_c` and K anchors
at 1 against **island-level** generation. One CENTRE-portable island constant (not a NI/RoI split — the
gap is a density/rurality confound, not a border effect); its spatial spread is carried as the K-prior
common-mode width `σ_mob`. See "Direct-K convex scale solve" for the prior.
...
motorcycle + taxi*, via the 23-cat `purpose_mapping.B01_COMPONENT`; **England level, before the
×`m_island` island rescale**): commute 0.200, retail 0.581
...
**Properly-derived K-prior (Λ_K — magnitude anchor + degeneracy break, derived not guessed).** The
inner K-solve is regularised by `(K−1)ᵀ Λ_K (K−1)` toward the generation value `1` (anchor fixed in
code), where **Λ_K = pinv(Cov_K)** and Cov_K's *shape* is derived from data, separating the two
epistemically-distinct directions and using each source only for what it measures well:
`Cov_K = P⊥·Cov_boot·P⊥ + σ_mob²·u·uᵀ + split_σ²·P⊥` (u = 1/√n·1 the common mode, P⊥ = I−uuᵀ the
differential/split subspace). **Split** (differential): `Cov_boot`, the PSU-cluster **bootstrap**
sampling covariance of the six log-rates (`generation_rates.json` `_meta.stat_cov`), common mode
projected out — it captures that the survey pins res/commute/retail levels far *tighter* than the old
flat 0.5 (~1.6%) and tertiary *looser*, plus real cross-correlations (res↔retail ≈0.7). **Level**
(common mode): `σ_mob` (`mobilisation.json`) — the spatial dispersion of island mobilisation (~0.3), a
loose leash the counts refine, so K departing from 1 honestly reads as local-vs-island mobilisation.
**Floor**: `split_σ` (`K_anchor_floor.split_sigma`, default 0.1) — the England→Ireland transfer
uncertainty on the purpose *mix* (the ~1.6% bootstrap CV is too confident to transfer the split at, so
this floors it) + a PSD backstop. Derived in log/relative space (`Var(K_c)≈Var(log ρ̂_c)`) but applied
as a linear-K cov about anchor 1 (first-order identical, keeps the convex linear-K Newton solve; Λ_K is
PSD ⇒ convex). Built once at setup over exactly `solve_scales`' comp_keys (res, commute, retail, active
school levels). It anchors magnitude, **and** — being loose along the level but tight on the split — it
pins the otherwise-flat commute↔retail direction, so no separate share prior is needed. Regularises the
inner K-solve only; **not** part of the reported χ². Walking obs mostly fall in slots with `f_school≈0`,
so the school-level scales are pinned almost entirely by the official school-peak hours; the prior
anchors but does not force them, so a genuine school over-fit still surfaces as a school `K_<level>`
departing from 1 (a diagnostic, not a masked one). **Purpose-mapping (commute↔retail) reallocation is
deliberately NOT in Λ_K**: under the unified schema a Business/PB relabel is magnitude-invariant (rate
+ temporal shape move together, total conserved); its only effect is *spatial* (which attractor field),
which must surface as fit residual, not be absorbed into the magnitude split — a cross-term there would
be a masking channel.
...
- **Component degeneracy:** the convex `solve_scales` could otherwise trade magnitude between
  similarly-shaped components; the properly-derived K-prior `(K−1)ᵀΛ_K(K−1)` (Λ_K loose along the
  common mode/level `σ_mob`, tight on the split from the bootstrap `stat_cov` + `split_sigma` floor)
  anchors every scale to the generation value 1 and pins the flat commute↔retail direction — see
  "Direct-K convex scale solve".
