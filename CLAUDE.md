...
| `simulation/census_supply.py` | `load_supply() → {area_code: {commute, school}}` — per-small-area trip *producers* harmonised NI (DZ) + RoI (SA): **commute** = residents who *drive* to work — car-driver modes only (RoI `SAP2022 T11T1` travel-to-work Car Driver + Van + Motorcycle; NI `transport_to_workplace` "Driving a car or van" + "Motorcycle, scooter or moped" + "Taxi"). WFH/not-in-employment are structurally excluded (separate travel-method categories, not selected). The car restriction matches the vehicle-driver modes in `derive_generation_rates.py`, since the model assigns car flow. **school** = student headcount, school-age + third-level, pre-school childcare excluded (matches the school attractor): RoI `T11T1` travel-to-school/college/childcare Total **minus** an age-0-4-distributed childcare estimate (`CHILDCARE_NATIONAL` = 130,000 ≈ CSO Census 2022 ~160k children aged 0-4 travelling to childcare/school, net of ~30k age-4 junior-infants kept as school; distributed ∝ SA age-0-4 from `SAP2022T1T1A`); NI `in_full_time_education` "Full-time student or schoolchild" (under-compulsory-age kids are "No code required" so childcare is already excluded — no subtraction). RoI key = JSON-stat SA-code label (the GUID index is ignored); a national "State" aggregate row (~2M) is dropped so per-SA values are clean 1×. Source data (gitignored): `data/ireland_census/` (RoI CSO SAP JSON-stat), `data/ni_census/` (NISRA DZ CSVs). |
...
| `simulation/tuner_config.json` | **Tracked in git.** Gravity param regularization and the scale K-prior. `gravity_lambda` + `gravity_ref` regularise the 4 willingness times (`TAU_res`/`TAU_commute`/`TAU_retail`/`TAU_school`, seconds; refs 480/1500/720/480); `K_prior_std` (per-component `res`/`commute`/`retail`/`school`, default 0.5) sets the width of the **generation-anchored K-prior** `Σ_c (K_c−1)²/σ_c²` inside `solve_scales` — softly pulls each component scale toward the generation value 1 (magnitude anchor + degeneracy break in one). The anchor is fixed at 1 in code (not a config knob); only the widths σ_c are configurable. `gamma_coupling_scale` and `lambda` are unused dead keys left in the file. External demand layers (workplace, retail_spaces, school_demand, commute/school producers) are measured per zone in `census_zones.json` — no external scale factors. |
...
| `analysis/driveshare.py` | **NEW.** `driveshare(d_miles)` → vehicle-driver **mode share** at trip length `d` miles — the empirical 0→peak rise of the production kernel (`driveShare(equiv_miles(c))` in `model._modesub_kernel`). Closed form `PLATEAU·(1−exp(−(d/D0)^K))` with authoritative module constants `PLATEAU=0.578, D0=1.26, K=1.26` (zero file I/O on import, scalar/numpy, monotone, `driveshare(0)=0`). `--fit` re-derives from NTS0308a (`data/nts0308.ods`, 2023/24, vehicle modes car/van-driver+motorcycle+taxi) applying three consistency corrections — **(1)** subtract NTS0409a "just walk" trips before the mode split (allocated across length bands ∝ the walk-mode length profile — the **key documented assumption**; no purpose×length cross-tab exists), **(2)** plateau-clip the England long-distance rail decline (`np.maximum.accumulate` — Ireland keeps car share flat), **(3)** origin-pinned band-AVERAGE least-squares fit (each band = the integral over the band, so the <1 mi sub-shape →0 at the origin, keeping `f(0)=0`) — prints the constants + writes `reports/driveshare.png`. Shared across all four components (mode/network property, not per-purpose); its **plateau cancels** in the production constraint. |
...
| `simulation/reset_gravity_params.py` | Reset the gravity params in `tuned_params.json` to the `gravity_ref` anchors in `tuner_config.json`: every `gravity_ref` shape param (TAU_res, TAU_commute, TAU_retail, THETA, TAU_school — iterates `gravity_ref`, so no rename drift) plus the four scales `K_res/K_commute/K_retail/K_sch` → 1.0. Strips dead keys (`K`, `K_biz`, `W_BIZ`, `W_SCHOOL`, `P_biz`, `ALPHA_biz`, the rational-kernel tail exponents `ALPHA`/`ALPHA_commute`/`ALPHA_retail`/`ALPHA_school`, the Tanner peak/shape `P`/`BETA`/`P_commute`/`BETA_commute`/`P_retail`/`BETA_retail`/`P_school`/`BETA_school`, `MU`, `SIGMA`). External params and `slot_fracs_*` are preserved. |
...
`simulation/node_weights.json` keys: `node_population`, `node_workplace` (all place-of-work jobs — feeds the external retail-spaces fallback + the map; **not** the commute attractor), `node_commute_attractor` (car-commute jobs = commute attractor, internal + external; from `census_attractor.py`), `node_retail_spaces` (estimated retail parking spaces = retail attractor, internal + external), `node_school_demand` (school places = school attractor), `node_commute_producers` / `node_school_producers` (census car-driving commuters / students = trip producers, internal + external; from `census_supply.py`), `boundary_node_ids` (auto-detected from core polygon). There is no combined `node_business_demand` layer — the commute (`node_commute_attractor`) and retail (`node_retail_spaces`) attractors are separate. External node entries (census-area-code string IDs, e.g. `"N21000219"`) are included alongside internal OSM node IDs.
...
- **Mode substitution** (the 0→peak rise) — `driveShare(equiv_miles(c))`: short trips are *walked,
  not driven*, so car demand is suppressed at short cost. This is a **mode/network** property →
  **shared across all components** and **empirically fixed** (not tuned). `driveShare(d_miles)` is the
  NTS0308a vehicle-driver mode share by trip length (`analysis/driveshare.py`,
  `0.578·(1−exp(−(d/1.26)^1.26))`); `equiv_miles(c)` converts OSRM seconds→miles
  (`analysis/equiv_miles.py`). `f(0)=0` holds (`equiv_miles→0 ⇒ driveShare→0`).
...
(`model._rational_kernel`/`model._tanner_kernel` + `model.gravity_assign` are retained for
back-compatibility / A-B revert; not used by the current production path.)
...
**Demand:** all layers are per-zone (Census 2021 DZ/SA aggregated to SDZ/DEA/ED/LEA), kept as **separate layers** (nothing is summed — each component uses its own producer/attractor, see "Four-component flow decomposition"). Producers: `commute_producers` (car-driving resident commuters) and `school_producers` (resident students), both census-derived, plus population. Attractors: `commute_attractor` (car-commute jobs per zone via `census_attractor.py` — commute attractor; `workplace` = all jobs is retained for the retail-spaces fallback + map, not as an attractor), `retail_spaces` (parking spaces within the zone via `parking_demand.parking_spaces` — retail attractor), and `school_demand` (per-zone OSM school enrolment via `school_demand.assign_enrolments` — school attractor).
...
- **School** (`flow_school`, kernel `τ_school`): home→school producer =
...
speed + network detour). Effect is **strongly kernel-dependent** (shorter willingness `τ_c` sharpens it): under a short-range kernel an external
...
`k_commuters`(commute_producers), `k_jobs`(commute_attractor), `k_retail`(retail_spaces),
...
**not** part of the reported χ². Walking obs mostly fall in slots with `f_school≈0`, so `K_sch` is
pinned almost entirely by the official school-peak hours; a soft K-prior anchors but does not force it,
so a genuine school over-fit still surfaces as `K_sch` departing from 1 (a diagnostic, not a masked one).
